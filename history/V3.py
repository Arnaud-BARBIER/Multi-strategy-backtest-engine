...
'''
This current version allows user to manually plug their strategy 
to the engine. This is not the final version, as I'd like user 
to simply wrap their strategy with the backtest imported from here.
'''
...
'''
Currently, to test a new strategy, we need to put the signal generation in this class
the strategy must return df with a column named Signal, that would be interpreted by 
the engine 
'''
...
engine = BacktestEngine.from_ticker(pipeline, "XAGUSD_M5", "2023-04-06", "2026-01-30", cfg)
trades = engine.run()
trades
