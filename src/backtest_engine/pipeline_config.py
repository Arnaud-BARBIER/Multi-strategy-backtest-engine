...
class Data:
    ...
    def load_ohlcv_csv(
        ...
        return self._build_ohlcv_name(asset, tf)
    ...
    def resample_ohlcv(
        ...
        out_open, out_high, out_low, out_close, out_volume = self.resample_ohlcv_arrays(
            open_=item.open,
            high=item.high,
            low=item.low,
            close=item.close,
            volume=item.volume,
            src_minutes=src_minutes,
        ...
        out_index = idx[::ratio]
        if drop_incomplete:
            out_index = out_index[:len(out_close)]
        else:
            out_index = out_index[:len(out_close)]
