...
def prepare_backtest_bundle(
    ...
    if regime_context is None and regime is not None and regime_policy is not None:
        regime_context = engine._resolve_strategy_regime_context(
            regime=regime,
    ...
    if (
        len(exit_profile_specs) > 0
        or len(exit_strategy_specs) > 0
        or position_binding is not None
    ):
...
class SignalPrep:
    mode: str = "winner"
    multi_out: dict[str, Any] = field(default_factory=dict)
    signals: np.ndarray | None = None
    selected_setup_id: np.ndarray | None = None
    selected_score: np.ndarray | None = None
    opens: np.ndarray | None = None
    ...
    lows: np.ndarray | None = None
    closes: np.ndarray | None = None
    market_signal_df: pd.DataFrame | None = None
    market_intent_pack: dict[str, np.ndarray] | None = None
