"""
╔══════════════════════════════════════════════════════════════════╗
║           NJIT PLOTS — post-run visualisation                    ║
║                                                                  ║
...
# ══════════════════════════════════════════════════════════════════
# 1. EDGE CURVE  ← métrique principale
# ══════════════════════════════════════════════════════════════════

def plot_edge(metrics: dict, label: str = "", show: bool = True) -> go.Figure:
    """
    Somme cumulative des retours individuels (même mise par trade).
    C'est l'edge brut sans effet compound — le vrai signal d'alpha.
    Utilise total_return_sum comme référence finale.
    """
    ...
    dd       = cum - roll_max      # drawdown absolu en % cumulé
...
def plot_equity(metrics: dict, label: str = "", show: bool = True) -> go.Figure:
    """Courbe compound — croissance du capital."""
    df       = metrics["trades_df"]
    ret      = df["return"].to_numpy()
    t        = df["exit_time"]
    ...
    fig.update_yaxes(title_text="Facteur", row=1, col=1)
    fig.update_yaxes(title_text="DD %",    row=2, col=1)
...
def plot_returns_dist(metrics: dict, bins: int = 60,
    ...
    """Histogramme wins vs losses, avec avg et profit factor."""
    df     = metrics["trades_df"]
    ...
    fig = _fig("Distribution des retours par trade", height=400)
    ...
    fig.update_xaxes(title_text="Retour %")
...
# ══════════════════════════════════════════════════════════════════
# 4. P&L PAR REASON DE SORTIE
# ══════════════════════════════════════════════════════════════════

def plot_by_reason(metrics: dict, show: bool = True) -> go.Figure:
    """Barres avg return + winrate par reason de sortie."""
    by = metrics.get("by_reason", {})
    if not by:
        print("by_reason vide — skip")
        return None
    ...
    fig = _fig("P&L par reason de sortie", height=400, rows=1, cols=2,
...
def plot_mae_mfe(metrics: dict, show: bool = True) -> go.Figure:
    """Scatter MAE vs MFE, colorié win/loss."""
    df = metrics["trades_df"]
    if "mae_intra" not in df.columns or df["mae_intra"].isna().all():
        print("MAE/MFE non disponible — track_mae_mfe=False ?")
        return None
    ...
    # ligne capture 100%  (y = x  dans un trade parfait)
...
# ══════════════════════════════════════════════════════════════════
# 6. RETOURS PAR PÉRIODE
# ══════════════════════════════════════════════════════════════════

def plot_period_returns(metrics: dict, show: bool = True) -> go.Figure:
    """Barres des retours agrégés par période (ME / WE / ...)."""
    df    = metrics["trades_df"]
    ...
    if len(series) == 0:
        print("Pas de données par période")
        return None
    ...
    fig.add_hline(y=0, line_color=_C["zero"], line_width=1)
    fig.update_yaxes(title_text="Retour %")
...
def print_summary(metrics: dict, label: str = "") -> None:
    """Résumé compact dans la console — toujours affiché."""
    m   = metrics
    sep = "─" * 55
    ...
    print(f"  ∑ retours (edge)  : {m.get('total_return_sum', 0)*100:+.2f}%  ← même mise/trade")
    ...
    print(f"  Périodes ({freq})    : {m.get('n_periods','?')}  pos={pp:.0%}")
    print(sep + "\n")


# ══════════════════════════════════════════════════════════════════
# 8. ENTRY PRINCIPALE — plot_results()
# ══════════════════════════════════════════════════════════════════
...
def plot_results(
    ...
    """
    Point d'entrée unique — affiche tous les graphes demandés.

    Paramètres
    ----------
    metrics    : dict retourné par engine.run()
    label      : texte libre affiché dans les titres
    edge       : ∑ retours bruts (même mise) — le vrai alpha signal
    equity     : courbe compound
    dist       : histogramme wins/losses
    by_reason  : barres par reason de sortie
    mae_mfe    : scatter MAE/MFE
    period_ret : barres par période
    summary    : résumé console

    Retourne
    --------
    dict {nom: go.Figure}  — pour éventuelle réutilisation
    """
    if metrics is None:
        print("metrics est None — aucun trade ?")
        return {}
