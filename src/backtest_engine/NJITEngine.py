...
class NJITEngine:
    ...
    def __init__(self, pipeline, ticker=None, start=None, end=None, cfg=None,
                atr_period=14, MAX_TRADES=50_000, MAX_POS=500, main_df=None):
        self.cfg        = cfg
        ...
        if main_df is not None:
            ...
            if "ATR" not in df.columns:
                df = pipeline.compute_atr(df, cfg.atr_period)
        else:
            if ticker is None:
    ...
    def inspect_signals(
        ...
        **kwargs,
        ...
        if setup_specs is not None:
            ...
            if plot:
                self._plot_signal_df(df, signal_col="Signal", crypto=crypto)
    ...
    def run(
        self,
        signals,use_exit_system=None,execution_context=None, features=None,bundle=None,
        ...
        phase_ev_from, phase_ev_to, phase_ev_profile, phase_ev_side = backtest_njit(
            ...
            setup_to_exit_profile, setup_to_exit_strategy,
            strategy_allowed_profiles, strategy_allowed_counts,
            self.minutes_of_day, self.day_index, self.day_of_week,
            ...
            max_tp, tp_period_mode, tp_period_bars, n_state_per_pos, n_state_global, n_strategies,
        ...
        if len(phase_ev_idx) > 0:
            phase_events_df = pd.DataFrame({
                ...
                "side":            phase_ev_side,
                "event_time":      self.bar_index[phase_ev_idx],
