...
def lag_regime_array(
    ...
    if lag == 0 or regime_arr.size == 0:
        return regime_arr.copy()

    out = np.empty_like(regime_arr)
    out[:lag] = np.int32(fill_value)
    out[lag:] = regime_arr[:-lag]
    return out


def make_regime_exit_policy(
    n_regimes: int,
    profile_by_regime: dict[int, int],
    strategy_by_regime: dict[int, int] | None = None,
    score_multiplier: dict[int, dict[str, dict[str, float]]] | None = None,
) -> RegimePolicy:
    return RegimePolicy(
        n_regimes=n_regimes,
        score_multiplier=score_multiplier or {},
        exit_profile_override=profile_by_regime,
        exit_strategy_override=strategy_by_regime or {},
    )
